...
namespace ApplicationLayer.EC2_3SYM
{
  ...
  {
    #region PRIVATE
    internal double[] values = new double[62]; //przechowywanie wartosci danych analogowych dla symulatora  
    ...
    class Const
    {
      ...
      internal const double P3zn   = 1100.0;
      ...
      internal const double Rkpc   =  69.0;
      internal const double Rkrc   =  59.0;
    ...
    {
      if(main) return coefficient*(values[(int)signalsIdx.Ao]-alfa)/Math.Pow( Math.Max(alfa, Azmin), 2);
      else
      {
    ...
    private void CountP(double Rkrz, double Rkrp, double F2p, double F3p, double dP23u, double P3u)
    {
      ...
      values[(int)signalsIdx.P3kpp ] = P3u + F3i.OVal * values[(int)signalsIdx.R3p];
